...
public class DataGrid extends LinearLayout {
	...
	private void initHeader() {
		...
		for (int i = 0; i < mc.COLUMN_STYLE.size(); i++) {
			...
			headerSpilterWidth = DataGridStyle.HeaderSpliterCell.LPadding
					+ DataGridStyle.HeaderSpliterCell.RPadding;
			dataSpliterWidth = DataGridStyle.SpliterCell.LPadding + DataGridStyle.SpliterCell.RPadding;
			WidthDifference = (i == 0 || i == mc.COLUMN_STYLE.size()) ? (headerSpilterWidth - dataSpliterWidth) / 2
					: headerSpilterWidth - dataSpliterWidth;
			HeaderView
					.setWidth((mc.COLUMN_STYLE.get(i).getWitdh() - WidthDifference));

			mc.HEADER_VIEW.add(HeaderView);

			mListHeader.addView(HeaderView, new LinearLayout.LayoutParams(
					android.view.ViewGroup.LayoutParams.WRAP_CONTENT,
					DataGridStyle.HeaderCell.Height));

			if (intCellSpliter < mc.COLUMN_STYLE.size()) {
				Spliter = new Splitter(getContext(), mc, i);
				Spliter.setIndex(i);
				mListHeader.addView(Spliter, new LinearLayout.LayoutParams(
						DataGridStyle.HeaderCell.Width, DataGridStyle.HeaderCell.Height));
				intCellSpliter++;
			}
		}
	}
	...
	public void create() {
		...
		//mListHeader.setBackgroundDrawable(getResources().getDrawable(
		...
		mTvNoData.setText(strNoDataText == null ? "Nenhum registro encontrado"
				: strNoDataText);
		mTvNoData.setGravity(Gravity.LEFT);
		mTvNoData.setTextColor(DataGridStyle.DataCell.TextColor);
	...
	public void addColumnStyle(String DisplayName, String strFieldName,
			int intDisplaySize) {
		mc.COLUMN_STYLE.add(new ColumnStyle(DisplayName, strFieldName,
				intDisplaySize));
	}
	...
	public void setLvOnCreateContextMenuListener(
