	...
	public boolean onTouchEvent(MotionEvent e) {
		...
		if (ScrollWidth != 0 && mc.DATAGRID_VIEW.getWidth() > WindowWidth) {
			...
			if (ScrollWidth > 0) {
				...
				if (ScrollWidth + WindowWidth > (mc.DATAGRID_VIEW.getWidth() - mc.DATAGRID_VIEW
						.getScrollX())) {
					balance = ScrollWidth
							+ WindowWidth
							- (mc.DATAGRID_VIEW.getWidth() - mc.DATAGRID_VIEW
									.getScrollX());
			...
			// Scroll to the left
				...
				if (mc.DATAGRID_VIEW.getScrollX() != 0) {
					...
					if (Math.abs(ScrollWidth) > mc.DATAGRID_VIEW.getScrollX()) {
						mc.DATAGRID_VIEW.scrollBy(
								-mc.DATAGRID_VIEW.getScrollX(), 0);
					} else
						mc.DATAGRID_VIEW.scrollBy(ScrollWidth, 0);
				}
			}
