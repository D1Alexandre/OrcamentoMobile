...
class Sort {
	...
	public void sortByColumn(int columnToSort, int sortOrder) {
		// sortResult
		Collections.sort(mc.DATA_SOURCE.getAllRows(),
				new ArrayColumnComparator(columnToSort, sortOrder));
	}

	private class ArrayColumnComparator implements
			Comparator<DataTable.DataRow> {
		...
		public int compare(DataTable.DataRow arg0, DataTable.DataRow arg1) {
			...
			if (sortOrder == SORT_ASC) {
				...
				if (str0 != null && str1 != null) {
					return arg0.get(columnToSortOn).compareTo(
							arg1.get(columnToSortOn));
				} else {
					return 0;
				}
			} else {
				...
				String str1 = arg1.get(columnToSortOn);
				if (str0 != null && str1 != null) {
					result = arg0.get(columnToSortOn).compareTo(
							arg1.get(columnToSortOn));
				}
				if (result > 0)
					return -1;
